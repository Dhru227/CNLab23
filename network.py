...
class Network:
    ...
    def __init__(self, netJsonFilepath, routerClass, visualize=False):
        ...
        self.clients = self.parseClients(netJson["clients"],
                                         self.clientSendRate)
    ...
    def parseLinks(self, linkParams):
        ...
        for addr1, addr2, latency in linkParams:
            ...
            links[(addr1, addr2)] = (latency, link)
        return links
    ...
    def addLinks(self):
        ...
        for addr1, addr2 in self.links:
            latency, link = self.links[(addr1, addr2)]
            if addr1 in self.clients:
                self.clients[addr1].changeLink(("up", link))
            if addr2 in self.clients:
                self.clients[addr2].changeLink(("up", link))
            if addr1 in self.routers:
                self.routers[addr1].changeLink(("up", addr2, link, latency))
            if addr2 in self.routers:
                self.routers[addr2].changeLink(("up", addr1, link, latency))
    ...
    def handleChanges(self):
        ...
        while not self.changes.empty():
            ...
            waitTime = (changeTime * self.latencyMultiplier + startTime) \
                       - currentTime
            if waitTime > 0:
            ...
            if change == "up":
                ...
                link = Link(addr1, addr2, latency, self.latencyMultiplier)
                self.links[(addr1, addr2)] = (latency, link)
                self.routers[addr1].changeLink(("up", addr2, link, latency))
                self.routers[addr2].changeLink(("up", addr1, link, latency))
            elif change == "down":
                addr1, addr2 = target
                _, link = self.links[(addr1, addr2)]
                self.routers[addr1].changeLink(("down", link))
                self.routers[addr2].changeLink(("down", link))
            #elif change == "cost":
            #    addr1, addr2, latency = target
            #    _, link = self.links[(addr1, addr2)]
            #    self.routers[addr1].changeLink(("cost", link, latency))
            #    self.routers[addr2].changeLink(("cost", link, latency))
