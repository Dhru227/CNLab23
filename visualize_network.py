...
class App:
    ...
    def drawLines(self):
        ...
        for addr1, addr2, latency in self.networkParams["links"]:
            line, lineLabel = self.drawLine(addr1, addr2, latency)
    ...
    def drawLine(self, addr1, addr2, latency):
        ...
        t = str(latency)
        label = self.canvas.create_text(tx, ty, text=t, state=NORMAL,
                                        font=tkFont.Font(size=self.networkParams["visualize"]["lineFontSize"]))
        return line, label
